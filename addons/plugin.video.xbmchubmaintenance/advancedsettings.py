''' kinkin '''
import xbmc,xbmcplugin,xbmcgui,os,sys,re,urllib,urllib2,settings,time,datetime
ADDON=settings.addon(); VIEW=settings.viewtype(); FILE_DIR=settings.xml_files()
addon_path=xbmc.translatePath(os.path.join('os.path.join(special://home','addons',''))
fanart=xbmc.translatePath(os.path.join('special://home','addons','plugin.video.xbmchubmaintenance','fanart.jpg'))
iconart=xbmc.translatePath(os.path.join('special://home','addons','plugin.video.xbmchubmaintenance','icon.png'))
SETTINGS_PATH=xbmc.translatePath(os.path.join('special://profile','addon_data','plugin.video.xbmchubmaintenance','settings.xml'))
USERDATA_PATH=xbmc.translatePath(os.path.join('special://profile',''))
ACTIVESETTINGSFILE=os.path.join(xbmc.translatePath('special://profile'),'advancedsettings.xml')
editfile=settings.xml_file()
def linkMENU(name): addDir("Advanced Settings Editor",'url',900,iconart,'blank','Build your advanced settings file')
def MENU(name):
...
def buildmenu(name):
    addDir("Troubleshooting settings ",'url',910,'','blank','Define your video settings')
    addDir("Audio/video playback settings",'url',911,'','blank','Define your audio settings')
    addDir("Video library settings",'url',912,'','blank','Define your video library settings')
    addDir("Library artwork",'url',913,'','blank','Define your library artwork settings')
    addDir("Video and music library settings",'url',914,'','blank','Define your video and music library settings')
    addDir("Music settings",'url',915,'','blank','Define your music settings')
    addDir("Photos settings",'url',916,'','blank','Define your photo settings')
    addDir("Network settings",'url',917,'','blank','Define your network settings')
    addDir("File system settings",'url',918,'','blank','Define your file system settings')
    addDir("Remote control settings",'url',919,'','blank','Define your remote control settings')
    addDir("Other interface settings",'url',920,'','blank','Define other interface settings')#Unsorted 
    addDir("Unsorted",'url',921,'','blank','Unsorted network settings')
    setView('movies','movies-view')
    xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
...
def resetsettings(name):
    ...
    for id,value in match:
        if value != 'DISABLED' and id != 'viewtype' and id != 'download-wallpaper' and id != 'email' and id != 'backup_path' and id != 'xbmc_path' and id != '7icon' and id != 'opened' and id != 'feed' and 'pc_' not in id:
...
def networkmenu(name):
    ...
    for d in dirlist:
        splitd=d.split('<>'); description=splitd[2]; currentsetting=ADDON.getSetting(str(splitd[0]))
        if '<>' in currentsetting: currentsetting=currentsetting.split('<>')[2]
        if currentsetting=='DISABLED': d="[COLOR red]%s[/COLOR] (%s)" % (splitd[0],currentsetting)
        else: d="[COLOR lime]%s[/COLOR] (%s)" % (splitd[0],currentsetting) 
        addDirPlayable(d,name,200,currentsetting,description,splitd[1],'')
    xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
    setView('movies','movies-view')
    xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
...
def video(name):
    ...
    for d in dirlist:
        splitd=d.split('<>'); description=splitd[2]; options=splitd[3]; currentsetting=ADDON.getSetting(str(splitd[0]))
        if '<>' in currentsetting: currentsetting=currentsetting.split('<>')[2]
        if currentsetting=='DISABLED': d="[COLOR red]%s[/COLOR] (%s)" % (splitd[0],currentsetting)
        else: d="[COLOR lime]%s[/COLOR] (%s)" % (splitd[0],currentsetting) 
        addDirPlayable(d,name,200,currentsetting,description,splitd[1],options)
    setView('movies','movies-view'); xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
def adjustrefreshrates(name):
    ...
    for d in dirlist:
        splitd=d.split('<>'); description=splitd[2]; options=splitd[3]; currentsetting=ADDON.getSetting(str(splitd[0]))
        if '<>' in currentsetting: currentsetting=currentsetting.split('<>')[2].replace('\n','').replace('&lt;','<').replace('&gt;','>').replace(' ','')
        if currentsetting=='DISABLED': d="[COLOR red]%s[/COLOR] (%s)" % (splitd[0],currentsetting)
        else: d="[COLOR lime]%s[/COLOR] (%s)" % (splitd[0],currentsetting) 
        addDirPlayable(d,name,200,currentsetting,description,splitd[1],options)
    setView('movies','movies-view'); xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
...
def edit_setting(name,url,iconimage,list,options):
    if 'COLOR' in name: name=regex_from_to(name,'COLOR','/COLOR').replace(' red','').replace(' lime','').replace('[','').replace(']','')
    if list=='num': data=keypad(name,iconimage)
    elif list=='num1': data=keypad_root(name,iconimage)
    elif list=='num2': data=keypad_root2(name,iconimage)
    elif list=='num3': data=keypad_root3(name,iconimage)
    elif list=='num_minus': data=keypad_minus(name,iconimage)
    elif list=='text': data=keyboard(name,iconimage)
    elif list=='bool': data=bool(name)
    elif list=='dialog': data=dialog(name,options)
    elif list=='override': data=override(name,options)
    elif list=='override_range': data=override_range(name,options)
    elif list=='fallback': data=fallback(name,options)
    elif list=='fallback_range': data=fallback_range(name,options)
    if len(data)>0:
        if data != 'DISABLED': data="%s<>%s<>%s" % (url,name,str(data))
        ADDON.setSetting(name,value=str(data)); xbmc.executebuiltin("Container.Refresh")
def write_xml(name,dir_path):
    count=0; readsettings=read_from_file(SETTINGS_PATH); settings=regex_get_all(readsettings,'<setting','/>')
    for s in settings:
        try:
            value=regex_from_to(s,'value="','"')
            if value != 'DISABLED' and 'download-wallpaper' not in s and 'email' not in s and 'backup_path' not in s and 'xbmc_path' not in s and '7icon' not in s and 'opened' not in s and 'feed' not in s and 'viewtype' not in s and 'pc_' not in s: count=count+1
        except: pass
    if count > 0:
        write_to_file(dir_path,'<!-- Created using the original HUB Maintenance tool -->\n<advancedsettings>\n',False)
        ...
        for s in settinglist: buildsection(s,dir_path)
        for s in settings:
            try: value=regex_from_to(s,'value="','"')
            except: pass
            if value != 'DISABLED' and 'download-wallpaper' not in s and 'email' not in s and 'backup_path' not in s and 'xbmc_path' not in s and '7icon' not in s and 'opened' not in s and 'feed' not in s and 'viewtype' not in s and 'pc_' not in s:
                rootid=value.split('&lt;&gt;')[0]; id=value.split('&lt;&gt;')[1]; id1=id.replace('_','')
                if '$' in id:
                    multi="two"; splitid=id.split('$'); id1=splitid[0]; id2=splitid[1]
                    try:
                        id3=splitid[2]
                        if len(id3)>0: multi='three'
                    except: pass
                else: multi="one"
                setting=value.split('&lt;&gt;')[2]
                if multi=='two': settingstring="    <%s>\n        <%s>%s</%s>\n    </%s>\n" % (id1,id2,setting.replace(' ','|'),id2,id1)
                elif multi=='three': settingstring="    <%s>\n        <%s>\n            <%s>%s</%s>\n        </%s>\n    </%s>\n" % (id1,id2,id3,setting.replace(' ','|'),id3,id2,id1)
                elif rootid=='root': settingstring="    <%s>%s</%s>\n" % (id1,setting,id1)
                    ...
                    if id=='delay': settingstring="    <%s>\n        <%s>%s\n        </%s>\n    </%s>\n" % ('video',rootid,setting,rootid,'video')
                    if rootid=='stagefright': settingstring="    <%s>\n        <%s>\n            <%s>%s</%s>\n        </%s>\n    </%s>\n" % ('video',rootid,id,setting,id,rootid,'video')
                else: settingstring="    <%s>\n        <%s>%s</%s>\n    </%s>\n" % (rootid,id1,setting,id1,rootid)
                if rootid not in str(settinglist): write_to_file(dir_path,settingstring,True) #!= 'video' and rootid != 'audio' and rootid != 'network' and rootid != 'edl' and rootid != 'pvr' and rootid != 'samba' and rootid != 'epg'
        write_to_file(dir_path,'</advancedsettings>',True)
    else: write_to_file(dir_path,'',False)
    notification('Maintenance Tool','File created','4000',iconart); xbmc.executebuiltin("Container.Refresh")
def buildsection(settingid,dir_path):
    countsettingid=0; setid=settingid.replace('&lt;&gt;',''); readsettings=read_from_file(SETTINGS_PATH); settings=regex_get_all(readsettings,'<setting','/>')
    for s in settings:
        try:
            value=regex_from_to(s,'value="','"')
            if str(settingid) in value: countsettingid=countsettingid+1
        except: pass
    if countsettingid > 0:
        ...
        for s in settings:
            ...
            if value != 'DISABLED' and 'download-wallpaper' not in s and 'email' not in s and 'backup_path' not in s and 'xbmc_path' not in s and '7icon' not in s and 'opened' not in s and 'feed' not in s and 'viewtype' not in s and 'pc_' not in s:
                print settingid,value; rootid=value.split('&lt;&gt;')[0]; id=value.split('&lt;&gt;')[1]; id=id.replace('_','')
                if '$' in id:
                    multi="two"; splitid=id.split('$'); id1=splitid[0]; id2=splitid[1]
                    try:
                        id3=splitid[2]
                ...
                setting=value.split('&lt;&gt;')[2]
                if rootid==setid:
                    if (id=='excludefromscan' or id=='excludefromlisting'): settingstring="        <%s>\n            <regexp>[-\._ ](%s)[-\._ ]</regexp>\n        </%s>\n" % (id,setting.replace(' ','|'),id)
                    elif multi=='two': settingstring="        <%s>\n            <%s>%s</%s>\n        </%s>\n" % (id1,id2,setting.replace(' ','|'),id2,id1)
                    elif multi=='three': settingstring="        <%s>\n            <%s>\n                <%s>%s</%s>\n            </%s>\n        </%s>\n" % (id1,id2,id3,setting.replace(' ','|'),id3,id2,id1)
                    else:
                        if ":" in id: idsplit=id.split(':'); settingstring='        <%s type="%s" path="%s/>\n' % (idsplit[0],idsplit[1],setting)
                        else: settingstring="        <%s>%s</%s>\n" % (id,setting,id)
                    write_to_file(dir_path,settingstring,True)
        write_to_file(dir_path,'    </%s>\n' % setid,True)
def keypad(name,csetting):
    data=xbmcgui.Dialog().numeric(0,'Enter '+name+' value'); data=int(data)
    if data>0: return str(data)
    else: return 'DISABLED'
def keypad_root(name,csetting):
    data=xbmcgui.Dialog().numeric(0,'Enter latency value'); data=int(data)
    if data>0: data="\n            <delay>%s</delay>" % data; return str(data)
    else: return 'DISABLED'
def keypad_root3(name,csetting):
    data=xbmcgui.Dialog().numeric(0,'Enter MIN rate (Hz) value'); minrate=int(data)
    if data>0:
        data=xbmcgui.Dialog().numeric(0,'Enter MAX rate (Hz) value'); maxrate=int(data)
        if data>0:
            data=xbmcgui.Dialog().numeric(0,'Enter delay value'); delay=int(data)
            if data>0:
                data="\n                <min>%s</min>\n                <max>%s</max>\n                <delay>%s</delay>" % (minrate,maxrate,delay); return str(data)
            else: return 'DISABLED'
        else: return 'DISABLED'
    else: return 'DISABLED'
def keypad_root2(name,csetting):
    data=xbmcgui.Dialog().numeric(0,'Enter rate (Hz) value'); rate=int(data)
    if data>0:
        data=xbmcgui.Dialog().numeric(0,'Enter delay value'); delay=int(data)
        if data>0:
            data="\n                <rate>%s</rate>\n                <delay>%s</delay>" % (rate,delay); return str(data)
        else: return 'DISABLED'
    else: return 'DISABLED'
def keypad_minus(name,csetting):
    data=xbmcgui.Dialog().numeric(0,'Enter '+name+' value'); data=int(data)
    if data>0: return "%s%s" % ('-',str(data))
    else: return 'DISABLED'
def keyboard(name,csetting):
    keyboard=xbmc.Keyboard(csetting,'Enter '+name+' value',False); keyboard.doModal()
    if keyboard.isConfirmed():
        data=keyboard.getText()
        if len(data) > 0: return data
        else: return 'DISABLED'
def bool(name):
    dialog=xbmcgui.Dialog(); action_list=["True","False","Disable"]; action_list_return=["true","false","DISABLED"]; action_id=dialog.select('Select '+name+' option',action_list); action=action_list_return[action_id]
    if(action_id < 0): return "DISABLED"
    else: return action
def override(name,csetting):
    keyboard=xbmc.Keyboard(csetting,'Enter fps value',False); keyboard.doModal()
    if keyboard.isConfirmed():
        fps=keyboard.getText()
        if len(fps) > 0:
            keyboard=xbmc.Keyboard(csetting,'Enter refresh value',False); keyboard.doModal()
            if keyboard.isConfirmed():
                refresh=keyboard.getText()
                if len(refresh) > 0:
                    data="\n                <fps>%s</fps>\n                <refresh>%s</refresh>" % (fps,refresh); return data
                else: return 'DISABLED'
        else: return 'DISABLED'
def override_range(name,csetting):
    keyboard=xbmc.Keyboard(csetting,'Enter MIN fps value',False); keyboard.doModal()
    if keyboard.isConfirmed():
        fpsmin=keyboard.getText()
        if len(fpsmin) > 0:
            keyboard=xbmc.Keyboard(csetting,'Enter MAX fps value',False); keyboard.doModal()
            if keyboard.isConfirmed():
                fpsmax=keyboard.getText()
                if len(fpsmax) > 0:
                    keyboard=xbmc.Keyboard(csetting,'Enter MIN refresh value',False); keyboard.doModal()
                    if keyboard.isConfirmed():
                        refreshmin=keyboard.getText()
                        if len(refreshmin) > 0:
                            keyboard=xbmc.Keyboard(csetting,'Enter MAX refresh value',False); keyboard.doModal()
                            if keyboard.isConfirmed():
                                refreshmax=keyboard.getText()
                                if len(refreshmax) > 0: data="\n                <fpsmin>%s</fpsmin>\n                <fpsmax>%s</fpsmax>\n                <refreshmin>%s</refreshmin>\n                <refreshmax>%s</refreshmax>" % (fpsmin,fpsmax,refreshmin,refreshmax); return data
                                else: return 'DISABLED'
                        else: return 'DISABLED'                
                else: return 'DISABLED'
        else: return 'DISABLED'
def fallback(name,csetting):
    keyboard=xbmc.Keyboard(csetting,'Enter fallback refresh value',False); keyboard.doModal()
    if keyboard.isConfirmed():
        refresh=keyboard.getText()
        if len(refresh) > 0: data="\n                <refresh>%s</refresh>" % (refresh); return data
        else: return 'DISABLED'
    else: return 'DISABLED'
def fallback_range(name,csetting):
    keyboard=xbmc.Keyboard(csetting,'Enter MIN fallback refresh value',False); keyboard.doModal()
    if keyboard.isConfirmed():
        refreshmin=keyboard.getText()
        if len(refreshmin) > 0:
            keyboard=xbmc.Keyboard(csetting,'Enter MAX fallback refresh value',False); keyboard.doModal()
            if keyboard.isConfirmed():
                refreshmax=keyboard.getText() 
                if len(refreshmax) > 0: data="\n                <refreshmin>%s</refreshmin>\n                <refreshmax>%s</refreshmax>" % (refreshmin,refreshmax); return data
                else: return 'DISABLED'
        else: return 'DISABLED'
def dialog(name,options):
    dialog=xbmcgui.Dialog(); list=regex_get_all(options,'"','"'); action_list=list; action_id=dialog.select('Select '+name+' option',action_list); action=action_list[action_id]
    if(action_id < 0): return "DISABLED"
    else: return action
def viewxml(name): msg=ACTIVESETTINGSFILE; TextBoxes("[B][COLOR lime]Your advancedsettings.xml file[/B][/COLOR]",msg)
...
def regex_from_to(text,from_string,to_string,excluding=True):
    if excluding: r=re.search("(?i)"+from_string+"([\S\s]+?)"+to_string,text).group(1)
    else: r=re.search("(?i)("+from_string+"[\S\s]+?"+to_string+")",text).group(1)
    return r
def regex_get_all(text,start_with,end_with): r=re.findall("(?i)"+start_with+"([\S\s]+?)"+end_with,text); return r
def strip_text(r,f,t,excluding=True): r=re.search("(?i)"+f+"([\S\s]+?)"+t,r).group(1); return r
def find_list(query,search_file):
    try: content=read_from_file(search_file); lines=content.split('\n'); index=lines.index(query); return index
    except: return -1
def add_to_list(list,file):
    if find_list(list,file) >= 0: return
    if os.path.isfile(file): content=read_from_file(file)
    else: content=""; lines=content.split('\n'); s='%s\n' % list
    for line in lines:
        if len(line) > 0: s=s+line+'\n'
    write_to_file(file,s); xbmc.executebuiltin("Container.Refresh")
def remove_from_list(list,file):
    index=find_list(list,file)
    if index >= 0:
        content=read_from_file(file); lines=content.split('\n'); lines.pop(index); s=''
        for line in lines:
            if len(line) > 0: s=s+line+'\n'
        write_to_file(file,s); xbmc.executebuiltin("Container.Refresh")
def write_to_file(path,content,append,silent=False):
    try:
        if append: f=open(path,'a')
        else: f=open(path,'w')
        f.write(content); f.close(); return True
    except:
        if not silent: print("Could not write to "+path)
        return False
def read_from_file(path,silent=False):
    try: f=open(path,'r'); r=f.read(); f.close(); return str(r)
    except:
        if not silent: print("Could not read from "+path)
        return None
def notification(title,message,ms,nart):
    xbmc.executebuiltin("XBMC.notification("+title+","+message+","+ms+","+nart+")")
def setView(content,viewType):
    if content: xbmcplugin.setContent(int(sys.argv[1]),content); xbmc.executebuiltin("Container.SetViewMode(504)")
def get_params(param=[]):
    ...
    return param
def addDir(name,url,mode,iconimage,list,description,suffix="",suffix2=""):
        u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&iconimage="+str(iconimage)+"&list="+str(list)+"&description="+str(description); ok=True
        liz=xbmcgui.ListItem(name+suffix+suffix2,iconImage=iconart,thumbnailImage=iconart); liz.setProperty('fanart_image',fanart)
        liz.setInfo(type="Video",infoLabels={"Title":name,'plot':description,'year':'2014','genre':'Advanced Setings'})
        setView('movies','movies-view'); #xbmc.executebuiltin("Container.SetViewMode(52)")
...
def addDirPlayable(name,url,mode,iconimage,description,list,options):
        u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&iconimage="+urllib.quote_plus(iconimage)+"&description="+str(description)+"&list="+str(list)+"&options="+str(options); ok=True
        liz=xbmcgui.ListItem(name,iconart,thumbnailImage=iconart); liz.setProperty('fanart_image',fanart)
        liz.setInfo(type="Video",infoLabels={"Title":name,'plot':description,'year':'2014','genre':'Advanced Setings'})
        setView('movies','movies-view'); #xbmc.executebuiltin("Container.SetViewMode(502)")
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz,isFolder=False); return ok
params=get_params()
url=None; name=None; mode=None; iconimage=None; options=None
...
try:    start=urllib.unquote_plus(params["start"])
except: pass
try:    list=urllib.unquote_plus(params["list"])
except: pass
try:    options=str(params["options"])
except: pass
try:    description=str(params["description"])
except: pass
#if mode==None or url==None or len(url)<1: #ADDON.setSetting('element',value='advancedsettings') #linkMENU(name)
if   mode==900: MENU(name)	
elif mode==499: buildmenu(name)
elif mode==498: checksettings(name)
elif mode==490: removexmlfile(name)
elif mode==489: resetsettings(name)
elif mode==910: troubleshooting(name)
elif mode==911: audiovideo(name)
elif mode==912: videolibrary(name)
elif mode==913: libraryartwork(name)
elif mode==914: videomusiclibrary(name)
elif mode==915: music(name)
elif mode==916: photos(name)
elif mode==917: networkmenu(name)
elif mode==918: filesystem(name)
elif mode==919: remotecontrol(name)
elif mode==920: interface(name)
elif mode==921: unsorted(name)
elif mode==101: jsonrpc(name)
elif mode==102: video(name)
elif mode==103: audio(name)
elif mode==104: edl(name)
elif mode==105: pvr(name)
elif mode==106: epg(name)
elif mode==115: video_library(name)
elif mode==202: adjustrefreshrates(name)
elif mode==205: latency(name)
elif mode==206: stagefright(name)
elif mode==300: networksettings(name)
elif mode==301: samba(name)
elif mode==302: network(name)
elif mode==303: tuxbox(name)
elif mode==200: edit_setting(name,url,iconimage,list,options)
elif mode==470: videodatabase(name)
elif mode==471: musicdatabase(name)
elif mode==472: musiclibrary(name)
elif mode==473: karaoke(name)
elif mode==495: viewxml(name)
elif mode==430: pathsubstitution(name)
elif mode==500: write_xml(name,list)
